...
import { sinkImages, ceramicSinks } from "@/lib/images";

export default function CeramicSinksPage() {
  return (
    <>
      <ParallaxImage src={sinkImages.hero} alt="Ceramic Sinks" height="70vh">
        <p
      ...
      <section className="bg-white">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-1 max-w-[1400px] mx-auto">
          {ceramicSinks.map((product, index) => (
            <ScrollReveal key={product.name} delay={index * 80}>
              <div className="img-zoom">
                <div className="relative aspect-[4/3]">
                  <Image src={product.image} alt={product.name} fill className="object-cover" sizes="(max-width: 768px) 100vw, (max-width: 1024px) 50vw, 33vw" />
                </div>
              </div>
